...
It determines how to assing a color to a given texture coordinate from the texture.
...
- The filter, that tells you how the point maps to the texture, it can be mapping to 
  ...
  combining the nearest pixels getting a blurred efect, or non-linearly combining them
  with the anisotropic filter, giving better texture quality in some cases.

- The addres mode, controls what happens when the texture coordinate gets out of bouds,
...
public:
	...
	Sampler(SAMPLE_FILTER filter, SAMPLE_ADDRESS_MODE address_mode);
